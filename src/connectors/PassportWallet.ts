...
export class NoPassportWalletError extends Error {
  public constructor() {
    super('Passport wallet not installed');
    this.name = NoPassportWalletError.name;
    Object.setPrototypeOf(this, NoPassportWalletError.prototype);
  }
}
 ...
 */
export interface PassportWalletConstructorArgs {
  actions: Actions;
  options?: Parameters<typeof detectEthereumProvider>[0];
  onError?: (error: Error) => void;
}

export class PassportWallet extends Connector {
  ...
  constructor({ actions, options, onError }: PassportWalletConstructorArgs) {
    super(actions, onError);
    this.options = options;
  }

  private async isomorphicInitialize(): Promise<void> {
    if (this.eagerConnection || !process.env.REACT_APP_PASSPORT_CLIENT_ID)
      return;
  ...
  public async activate(
    ...
    return this.isomorphicInitialize()
      .then(async () => {
        if (!this.provider) throw new NoPassportWalletError();
